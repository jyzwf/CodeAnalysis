    ...
    function match(
        ...
        if (name) {
            ...
            for (let i = 0; i < pathList.length; i++) {
                ...
                if (matchRoute(record.regex, location.path, location.params)) { // 找到当前location.path 匹配的 Record
                    // 由于 pathList 是从底层往上记录，所以这里只要找到一个底层即可，
                    return _createRoute(record, location, redirectedFrom);
                }
            }
        }
    ...
    function redirect(record: RouteRecord, location: Location): Route {
        ...
        let redirect =
            typeof originalRedirect === 'function'
                ? originalRedirect(createRoute(record, location, null, router)) // https://router.vuejs.org/zh/guide/essentials/redirect-and-alias.html#%E9%87%8D%E5%AE%9A%E5%90%91
                : originalRedirect;
        ...
        if (!redirect || typeof redirect !== 'object') {
            if (process.env.NODE_ENV !== 'production') { //主要是自定义redirect时没有返回值的情况
                warn(
                    false,
        ...
        const { name, path } = re;
        let { query, hash, params } = location; // 从redirect中去相应的数据
...
function matchRoute(regex: RouteRegExp, path: string, params: Object): boolean {
    ...
    if (!m) {
        ...
        // 关于字符串的match方法的length，为匹配到的数组长度
