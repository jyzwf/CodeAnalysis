...
export function createCompiler(baseOptions: CompilerOptions) {
  ...
  */
  ...
  function compile(
    template: string,
    options?: CompilerOptions   // 自己的一套options
  ): CompiledResult {
    ...
    if (options) {
      ...
      if (options.directives) {
        // directives 是对象
        /*合并directives
          覆盖baseOptions 的同名directives
          防止对象引用问题，不然finalOptions的directives与options 的directives指向相同，
          当options.directives改变时，finalOptions.directives也会改变
        */
  ...
  function compileToFunctions(
    ...
    const key = options.delimiters ?
      ...
      template
    if (functionCompileCache[key]) { // 保存每次缓存的结果 --> 这里岂不是改变一个空格都要重新编译？
      return functionCompileCache[key]
    }

    // compile
    /*编译*/
    const compiled = compile(template, options) // 上述的compile
    ...
    const res = {}
    const fnGenErrors = []  // 收集 makeFunction时的错误
