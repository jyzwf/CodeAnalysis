...
// 匹配@click.stop、@keyup.enter等属性中的修饰符
const modifierRE = /\.[^.]+/g;
...
export function parse(
  ...
  platformGetTagNamespace = options.platformGetTagNamespace || no; // 获取tag的命名空间，svg或math
  platformMustUseProp = options.mustUseProp || no; // 判断是否需要通过绑定prop来绑定属性
  /*检测是否是<pre>标签*/
  platformIsPreTag = options.isPreTag || no;
  // 收集各个模块在transforms 前、中、后要干的事情
  ...
  parseHTML(template, {
    ...
    start(tag, attrs, unary) {
      ...
      const element: ASTElement = {
        ...
        tag,
        attrsList: attrs,
        attrsMap: makeAttrsMap(attrs), // attr.name -> attr.value 的映射
      ...
      if (!inVPre) {
        ...
        */
        processPre(element); // 这里判断v-pre 是否存在于element的attrs中，如果存在，就会element.pre = true，表示存在v-pre指令
        if (element.pre) {
          inVPre = true;
        }
      }
      ...
      if (!unary) {
        ...
        // 单元素标签不需要入栈，并且如果单元素是pre标签或者有 v-pre 属性，就在此置为 false
        endPre(element);
      }
...
*/
      ...
      /*该子节点是非<pre>标签的文本*/
      // 这是因为我们的模板一般都会缩进，都会有换行，所以这里是清除换行等添加的内容。
    ...
    chars(text: string) {
      ...
      text =
        inPre || text.trim()
          ? isTextTag(currentParent) // tag === script/style
            ? text
...
function processFor(el) {
  ...
  if ((exp = getAndRemoveAttr(el, "v-for"))) {
    ...
    /*
      因为item可能是被括号包裹的，比如(item, index, l) in sz这样的形式，匹配出这些项
      例：(item, index)匹配得到结果
      [ '(item, index)',
      'item',
      ' index',
...
function processIfConditions(el, parent) {
  /*当遇到当前el有v-else或者v-elseif属性的时候，需要处理if属性，在其上级兄弟元素中必然存在v-if属性*/
...
function processAttrs(el) {
  ...
  for (i = 0, l = list.length; i < l; i++) {
    ...
    if (dirRE.test(name)) {
      ...
      el.hasBindings = true;
      // modifiers
      /*解析表达式(修饰符)，比如 :a.b.c.d得到结果{b: true, c: true, d:true}*/
      ...
      if (bindRE.test(name)) {
        ...
        name = name.replace(bindRE, ""); // name -> a
        /*解析过滤器*/
        ...
        // parse arg
        // argRE = /:(.*)$/
        // 传给指令的参数，可选。例如 v-my-directive:foo.a.b 中，参数为 "foo"
        const argMatch = name.match(argRE);
        //  arg = 'foo'
        const arg = argMatch && argMatch[1];
        if (arg) {
          name = name.slice(0, -(arg.length + 1)); // name = my-directive
        }
        /*将参数加入到ele的directives中去*/
        addDirective(el, name, rawName, value, arg, modifiers);
        if (process.env.NODE_ENV !== "production" && name === "model") {
          // http://hcysun.me/vue-design/art/83vue-parsing-2.html
