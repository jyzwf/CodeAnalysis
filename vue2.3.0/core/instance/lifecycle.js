...
export function initLifecycle(vm: Component) {
  ...
  if (parent && !options.abstract) {
    // 抽象组件才有 abstract ： true
    // 跳过抽象组件
    while (patrent.$options.absract && parent.$parent) {
      parent = parent.$parent;
    }
    parent.$children.push(vm);
  }

  vm.$parent = parent; // 指向父组件的实例
  // 没有父级就表示自己就是根，否则一层一层往上找
  vm.$root = parent ? parent.$root : vm; // 指向根vm实例
...
export function mountComponent(
  ...
  // 在这里的watch 里面的 get 函数触发 updateComponent ，进行页面的渲染
  // 只收集页面渲染所需的data，直接执行 updateComponent 函数
