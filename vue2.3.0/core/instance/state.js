...
function initProps(vm: Component, propsOptions: Object) {
  const propsData = vm.$options.propsData || {}; // 只用于 new 创建的实例中。
...
function initData(vm: Component) {
  ...
  data = vm._data =
    typeof data === "function" // data 以函数的形式返回
      ? getData(data, vm)
      : data || {};
  ...
  if (typeof userDef === "function") {
    ...
    */
    ...
    sharedPropertyDefinition.get = userDef.get
      ? userDef.cache !== false // https://juejin.im/post/5a6f21a86fb9a01ca47ac4b9
...
function createComputedGetter(key) {
  return function computedGetter() {
    ...
    if (watcher) {
      ...
      if (watcher.dirty) {
        // dirty一定是true,因为之前的lazy 传入是就是true
        watcher.evaluate();
      }
      /*依赖收集*/
      // 该次触发的上一个dep_target
