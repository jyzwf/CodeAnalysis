...
export default class Watcher {
  ...
  constructor(
    ...
    if (options) {
      ...
      this.user = !!options.user    // 用户自定义的使用$watch产生的watch
    ...
    this.active = true  // 表示该watcher 是否被卸载
  ...
  addDep(dep: Dep) {
    ...
    if (!this.newDepIds.has(id)) {
      ...
      // newDeps和newDepIds是当前这次触发时的，
      // deps/depIds是上一个上一次触发时的
      if (!this.depIds.has(id)) {   // 这里是再次出发getter函数的时候，用来判断属性的dep是否加了这个watcher
        dep.addSub(this)
      }
    }
  }
  ...
  cleanupDeps() {
    ...
    while (i--) {
      ...
      if (!this.newDepIds.has(dep.id)) {  // 获取最新一份能够触发该watcher的属性列表
        dep.removeSub(this)
      }
    }
