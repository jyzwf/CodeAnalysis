...
strats.data = function (
  ...
  if (!vm) {
    ...
    if (typeof childVal !== 'function') { // data 属性必须是函数
...
function checkComponents(options: Object) {
  for (const key in options.components) {
    ...
    if (isBuiltInTag(lower) || config.isReservedTag(lower)) { // 内置标签或者保留标签
      // isBuiltTag:slot,component
      // isReservedTag:isHTMLTag(tag) || isSVG(tag)
      warn(
...
export function mergeOptions(
  ...
  */
  ...
  if (child.mixins) {   // 这里先将mixin里的和父级的options 根据合并策略进行合并，然后再合并自己和父级的options
  ...
  const options = {}
  let key
  for (key in parent) { // 父节点有，子节点有？子节点合并父节点：父节点
    mergeField(key)     // 这里解决了父节点与子节点如果有相同的属性，则以子节点为主，否则就直接返回父节点属性
  }
  /*合并parent与child*/
  for (key in child) {  // 子节点特有的属性
